...
class BlockNode: GameObject {
    ...
    init(position: CGPoint, number: Float) {
        ...
        parentNode = SKSpriteNode(color: .clear, size: CGSize(width: number >= 1000 || number <= -1000 ? 156 : 76, height: 76))
        parentNode.position = CGPoint(x: position.x, y: position.y)
        
        // Quadrado
        let node = SKShapeNode(rectOf: CGSize(width: number >= 1000 || number <= -1000 ? 156 : 76, height: 76), cornerRadius: 9)
        ...
        // Para Renderizar
        parentNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: number >= 1000 || number <= -1000 ? 166 : 83, height: 83))
